...
public class PAEEController implements Initializable {
    ...
    private void desabilitarCamposPorStep() {
        ...
        System.out.println("Desabilitando campos específicos por step: " + currentStep);
        
        // Desabilita campos específicos baseados no step atual
        switch (currentStep) {
            case 1:
                // Step 1: Resumo e checkboxes de dificuldades
                if (resumoCaso != null) {
                    resumoCaso.setEditable(false);
                }
                desabilitarChoiceBox(dificuldadesMotoresPsicomotoresCb);
                desabilitarChoiceBox(dificuldadesCognitivoCb);
                desabilitarChoiceBox(dificuldadesSensorialCb);
                desabilitarChoiceBox(dificuldadesLinguagemComunicacaoCb);
                desabilitarChoiceBox(dificuldadesFamiliarCb);
                desabilitarChoiceBox(dificuldadesAfetivoInterpessoaisCb);
                desabilitarChoiceBox(dificuldadesRaciocinioLogicoMatematicoCb);
                desabilitarChoiceBox(dificuldadesAVAsCb);
                break;
                
            case 2:
                // Step 2: Desenvolvimento motor e comunicação
                desabilitarTextArea(desenvolvimentoMotoresPsicomotoresDificuldadesTa);
                desabilitarTextArea(desenvolvimentoMotoresPsicomotoresIntervencoesTa);
                desabilitarTextArea(comunicacaoLinguagemDificuldadesTa);
                desabilitarTextArea(comunicacaoLinguagemIntervencoesTa);
                break;
                
            case 3:
                // Step 3: Raciocínio e atenção
                desabilitarTextArea(dificuldadesRaciocinioTa);
                desabilitarTextArea(intervencoesRaciocinioTa);
                desabilitarTextArea(dificuldadesAtencaoTa);
                desabilitarTextArea(intervencoesAtencaoTa);
                break;
                
            case 4:
                // Step 4: Memória e percepção
                desabilitarTextArea(dificuldadesMemoriaTa);
                desabilitarTextArea(intervencoesMemoriaTa);
                desabilitarTextArea(dificuldadesPercepcaoTa);
                desabilitarTextArea(intervencoesPercepcaoTa);
                break;
                
            case 5:
                // Step 5: Sociabilidade e AVAs
                desabilitarTextArea(dificuldadesSociabilidadeTa);
                desabilitarTextArea(intervencoesSociabilidadeTa);
                desabilitarTextArea(dificuldadesAVATa);
                desabilitarTextArea(intervencoesAVATa);
                break;
                
            case 6:
                // Step 6: Objetivos e encaminhamentos
                desabilitarTextArea(objetivosPlano);
                desabilitarChoiceBox(cbAEE);
                desabilitarChoiceBox(cbPsicologo);
                desabilitarChoiceBox(cbFisioterapeuta);
                desabilitarChoiceBox(cbPsicopedagogo);
                desabilitarChoiceBox(cbTO);
                desabilitarChoiceBox(cbEducacaoFisica);
                desabilitarChoiceBox(cbEstimulacaoPrecoce);
                break;
        }
    }

    // Métodos auxiliares para desabilitar campos específicos
    private void desabilitarTextArea(TextArea textArea) {
        if (textArea != null) {
            textArea.setEditable(false);
            textArea.setStyle("-fx-background-color: #f0f0f0; -fx-text-fill: #666;");
        }
    }

    private void desabilitarChoiceBox(ChoiceBox<String> choiceBox) {
        if (choiceBox != null) {
            choiceBox.setDisable(true);
            choiceBox.setStyle("-fx-opacity: 1; -fx-background-color: #f0f0f0;");
        }
    }
